...
type EcranUI = 'menu' | 'jeu' | 'pause' | 'fin';
...
export class GameApp {
  ...
  private placePatinoire(): void {
    ...
    this.decor = {
      glace: construitGlace(nouveau, this.W, this.H),
      foule: construitFoule(nouveau, this.W, this.H),
      traces,
    };
  }
  ...
  private rendu(): void {
    ...
    if (this.portrait || !this.state || !this.rink || !this.decor) {
      ...
      dessineScene(g, this.rink, state, this.decor, this.sprites, this.effets, this.ecranUI);
      g.setTransform(1, 0, 0, 1, 0, 0);
      if (this.ecranUI !== 'menu') dessineTableau(g, this.W, state, this.ecranUI);
